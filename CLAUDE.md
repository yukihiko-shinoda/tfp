...
test` collection to the top-level `tests/` directory — see Architecture below for why `tfpcli/tests/`
...
TFP doesn't hand-roll its own multiprocessing. `tfpcli/cli.py`'s `main()` shells out to
`pytest.main()`, pointed at the `tfpcli/tests/` directory bundled *inside the installed package* (not
the top-level `tests/` directory, which holds TFP's own unit tests):

```
tfp <project> -n 4
  -> pytest.main([<path to tfpcli/tests/>, "--prj", "<project>", "-n", "4"])
```

- [tfpcli/tests/conftest.py](tfpcli/tests/conftest.py) adds the `--prj` option and, via
  `pytest_generate_tests`, parametrizes every "test" over each environment defined for that project
  in `tfp.yml` (loaded through [tfpcli/config.py](tfpcli/config.py)'s `Config`/`Project` dataclasses).
- [tfpcli/tests/test_plan.py](tfpcli/tests/test_plan.py) has exactly one test function, `test()`. Each
...
- [tfpcli/tests/testlibraries/runner.py](tfpcli/tests/testlibraries/runner.py)'s `Runner` renders each
...
- `-n auto` (the CLI default) is resolved by `tfp` itself, not passed through to pytest-xdist
  verbatim: `Config.projects[project].numprocesses_auto()` in `tfpcli/config.py` computes
...
**Consequence for this codebase**: `tfpcli/tests/` looks like a test package but is production code —
...
[tfpcli/report.py](tfpcli/report.py) (`tfp-report` entry point) is a separate concern from the plan
...
- `tests/` (top level) — TFP's own unit/integration tests for `tfpcli.cli`, `tfpcli.config`,
  `tfpcli.report`. Standard pytest, collected by default.
- `tfpcli/tests/` (inside the package) — shipped as part of the installed `tfpcli` package; this is
  the pytest suite that `tfpcli/cli.py` invokes at runtime against a *user's* Terraform project,
  described above. Excluded from default collection.

Keep this distinction in mind when adding tests: a bug fix to plan-execution behavior belongs under
`tfpcli/tests/`, while a bug fix to the CLI/config/report layer belongs under the top-level `tests/`.
